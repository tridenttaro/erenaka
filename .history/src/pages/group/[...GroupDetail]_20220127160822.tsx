...
const GroupDetail = (props: Props) => {
  ...
  const groupId1 = queryStr ? queryStr[0] : "";
  const currentDirectory1 =
    queryStr && queryStr.length > 1 ? queryStr.slice(1) : [];
  console.log(`GroupId: ${groupid1}__cd: ${JSON.stringify(currentDirectory1)}`);
  ...
  // if (!groupLoading && groupsInfo.length < 1) setUrlError(true);
  ...
  return (
    <div>
      ...
      {groupLoading ? (
        ...
        <div
          style={{ margin: "0 auto", textAlign: "center", fontWeight: "bold" }}
        >
          <p>不正なURLです</p>
        </div>
      ) : (
