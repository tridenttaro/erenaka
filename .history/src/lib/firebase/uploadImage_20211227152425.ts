...
const uploadImage = async (props: Props) => {
  ...
  const { image, userState, groupId, currentDirectory } = props;

  let storagePath = "temp";
  if (props.groupId) {
    storagePath = `groups/${props.groupId}`;

    if (props.currentDirectory && props.currentDirectory.length > 0) {
      storagePath += `/${props.currentDirectory.join("/")}`;
    }
  }
