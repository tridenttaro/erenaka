...
const downloadFile = async (props: Props) => {
  if (!props.userState && !props.upToGroupId && props.userState) return;

  const { downloadKey, userState } = props;
  ...
  try {
    ...
    // 名刺データの取得
    const businessCardData = { company: "" };
    ...
    await uploadImage({ image, userState, businessCardData });
